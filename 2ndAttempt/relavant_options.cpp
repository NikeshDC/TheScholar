...
int main()
{
    ...
    int result_size_name = 0;
    int result_size_genre = 0;
    int result_size_author = 0;
    std::vector<std::string> results;
    std::string keyword;
    char c = 'a';
    std::cout<<"\nENTER KEYWORD TO GET RELEVANT BOOK NAMES FOR ($ TO EXIT)\n\n";
    while((c = getch())!='$')
    {
        keyword.push_back(c);
        std::cout<<"\n\nkeyword: "<<keyword;
            getRelevantBookNames(keyword, results, result_size_name);
            //getRelevantBookGenres("c", results, result_size_genre);
            //getRelevantBookAuthors("a", results, result_size_author);
